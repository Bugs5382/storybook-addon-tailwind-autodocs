...
export const resolveConfig = (fileName: string): ResolvedConfig => {
    const cssContent = readFileSync(fileName, 'utf-8');
    const customTheme = parseThemeFromCSS(cssContent);
    ...
    return {
        theme: {
            colors: deepMerge(baseColors, customTheme.colors || {}),
            fontSize: deepMerge(
                defaultTheme.fontSize || {},
                customTheme.fontSize || {}
            ),
            fontFamily: deepMerge(
